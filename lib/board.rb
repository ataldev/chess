...
class Board
  ...
  def self.start_chess
    board = self.new
    8.times do |c|
      board[[1,c]] = Pawn.new(board, [1, c], :white)
      board[[6,c]] = Pawn.new(board, [6, c], :black)
    end
    
    [[0, :white], [7, :black]].each do |(r, color)|
    ...
    board[[0, 3]] = King.new(board, [0, 3], :white)
    board[[7, 3]] = King.new(board, [7, 3], :black)

    board[[0, 4]] = Queen.new(board, [0, 4], :white)
    board[[7, 4]] = Queen.new(board, [7, 4], :black)
